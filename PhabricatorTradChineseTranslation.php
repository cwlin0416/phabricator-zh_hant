...
final class PhabricatorTradChineseTranslation
  ...
  'Change primary email address?' => '更改主要電子郵件地址?',
  ...
  'Identity' => '識別',
  'If you change your primary address, Phabricator will send all email to %s.' => '若您更改您的主要地址，Phabricator 將會傳送所有電子郵件到 %s。',
...
要取得完整的編輯器設定操作說明，請參考 **[[ %s | %s ]]**。',
  ...
  'Note: Changing your primary email address will invalidate any outstanding password reset links.' => '注意：更改您的主要電子郵件地址會作廢所有已寄出的密碼重設連結。',
  'Note: Removing an email address from your account will invalidate any outstanding password reset links.' => '注意：從您的帳號移除電子郵件地址會作廢所有已寄出的密碼重設連結。',
  'Old Password' => '舊密碼',
  ...
  'Pronoun' => '代名詞',
  'Really delete address \'%s\'?' => '確定要刪除地址 \'%s\'?',
...
這些偏好設定 **僅會** 套用到您所連結到的物件 (例如，您為修訂中的審查員或您在 CC 的工作)。當其他物件被建立、設定時要接收郵件通知，請設定 [[ /herald/ | Herald 規則 ]]。',
  ...
  'You have no account setup issues.' => NULL,
  ...
  'Your primary email address is unverified. You will not be able to receive email until you verify it.' => NULL,
  '◐ Notify' => '◐ 通知',
  '⚪ Ignore' => '⚪ 忽略',
  '⚫ Email' => '⚫ 郵件通知',
...
再複雜一點的設定可讓使用者用其登入名稱或電子郵件地址登入：

  lang=text,name=符合多個屬性
  mail
  sn

若您的 LDAP 目錄結構較複雜，或者您想要執行較複雜的篩選，您可以使用更複雜的查詢條件。依您的目錄結構而定，這個範例可讓指定部門的使用者用其電子郵件地址或使用者名稱登入：
...
The address will automatically be verified, so you do not need to be able to receive mail at this address, and can enter some invalid or nonexistent (but correctly formatted) address like `bot@yourcompany.com` if you prefer.' => '**為何機器人帳號需要電子郵件地址?**

雖然機器人一般不會接受到來自 Phabricator 的郵件，但會與其他需要電子郵件地址的系統互動，例如：

  - 若這個帳號做的動作會 //發送// 郵件，我們會需要使用郵件地址來填寫 //寄件者// 標頭。
  - 若這個帳號建立了提交，Git 與 Mercurial 會需要郵件地址來記錄作者。
  - 若您用代表帳號的郵件地址發送郵件到 //到// Phabricator，系統可以透過郵件地址來辨識寄件者。
  - 部份內部認証功能會使用到帳號的郵件地址。


這個郵件地址會自動被驗証通過，所以您無須到此地址接收郵件，若您想要也可以輸入無效或不存在的地址如 `bot@yourcompany.com`。',
...
Phabricator' => NULL,
  ...
  'This email address (%s) is no longer your primary email address. Going forward, Phabricator will send all email to your new primary email address (%s).' => '此電子郵件地址 (%s) 已不再是您的主要電子郵件地址，往後 Phabricator 將會寄送所有電子郵件到您新的主要電子郵件地址 (%s)。',
...
%s' => NULL,
  'This is now your primary email address (%s). Going forward, Phabricator will send all email here.' => '現在這是您的主要電子郵件地址 (%s)，往後 Phabricator 將會寄送所有電子郵件到這裡。',
